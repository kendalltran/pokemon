...
# (A1) ^importing the Flask class.  this class, once made into an object, will implement a WSGI application
# (D1) ^importing render_template allows us to utilize html files in the 'templates' folder
# (E1) ^importing request allows us to retrieve attributes of the GET or POST request
...
app = Flask(__name__)
# (A2) ^create instance of Flask class.  use __name__ (generic) if single module is used
...
engine = create_engine('sqlite:///pokemonCenter.db', connect_args={'check_same_thread': False})
Base.metadata.bind = engine
DBSession = sessionmaker(bind=engine)
session = DBSession()
# (B1) ^connect to database and create database session via SQLAlchemy built-in functions
...
def gconnect():
    ...
    def createUser(login_session):
        newUser = User(username=login_session['username'], useremail=login_session['email'], picture = login_session['picture'])
        session.add(newUser)
        session.commit()
...
def viewUserJSON(userid):
    ...
    # (C1) ^returns serialized date in JSON format (useful for other applications to retrieve/utilze this data)
...
def viewKanto():
    ...
    if 'username' not in login_session:
        ...
        return render_template('users.html', users = users, loggedInUserId = loggedInUserID)
...
def addUser():
    ...
    if request.method == 'POST':
        newUser = User(username = request.form['name'], useremail = request.form['email'], picture = request.form['picture'])
        session.add(newUser)
        session.commit()
...
def viewPokemons(userid):
    ...
    if login_session['user_id'] != user.userid:
        return render_template('publicpokemons.html', user = user, pokemons = pokemons)
    else:
        return render_template('pokemons.html', user = user, pokemons = pokemons)
...
def addPokemon(userid):
    ...
    user = session.query(User).filter_by(userid = userid).one()
    if request.method == 'POST':
        newPokemon = Pokemon(pokemonname=request.form['name'], user_id = login_session['user_id'], picture=request.form['picture'], gender=request.form['gender'])
...
@app.route("/kanto/<int:userid>/pokemons/<int:pokemonid>/edit", methods = ['GET', 'POST'])
def editPokemon(userid, pokemonid):
    # user = session.query(User).filter_by(userid = userid).one()
    if 'username' not in login_session:
        return redirect('/login')
    pokemon = session.query(Pokemon).filter_by(user_id = userid, pokemonid = pokemonid).one()
    if request.method == 'POST':
...
@app.route("/kanto/<int:userid>/pokemons/<int:pokemonid>/delete", methods = ['GET', 'POST'])
...
# supporting links and top-level notes:
